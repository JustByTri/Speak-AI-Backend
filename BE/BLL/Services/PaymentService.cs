...
namespace Service.Service
{
    public class PaymentService : IPaymentService
    {
        ...
        public async Task<PaymentResponseDTO> CreatePaymentRequest(PaymentRequestDTO paymentInfo, HttpContext context)
        {
            try
            {
                ...
                if (order == null)
                {
                    ...
                    {
                        StatusCode = 400,
                        Message = "Order không tồn tại hoặc không ở trạng thái Pending.",
                        IsSuccess = false
                    };
                }
                ...
                return new PaymentResponseDTO
                {
                    ...
                    IsSuccess = true,
                    Result = await _vnpPayService.CreatePaymentUrl(paymentRequest, paymentInfo.VoucherCode ?? "", context)
                };
            }
            catch (Exception ex)
            {
